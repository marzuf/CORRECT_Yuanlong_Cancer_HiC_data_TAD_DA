...
# Rscript corr_logFC_empPvals_withWilcox.R 

startTime <- Sys.time()

cat("> START corr_logFC_empPvals_withWilcox.R \n")
...
outFolder <- file.path("CORR_LOGFC_EMPPVALS_WITHWILCOX")
...
if(build_signifTADs_allDS_data){
  ...
  allPvals_allDS_DT <- foreach(ds = all_hicexpr_ds, .combine='rbind') %dopar% {
    ...
    outDT <- data.frame(
      ...
      pvalWilcox = tad_pvalWilcox[all_tads],
