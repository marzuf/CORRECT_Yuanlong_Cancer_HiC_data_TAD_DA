...
cat("> START cumulative_features_abs.R \n")
...
outFold <- "CUMULATIVE_FEATURES_ABS"
dir.create(outFold)
...
foo <- foreach(fc_file = all_tadFC_files) %dopar% {
  ...
  outFile <- file.path(outFold, paste0(hicds, "_", exprds, "_", "cumulative_abs_fc_tads.", plotType))
  ...
  outFile <- file.path(outFold, paste0(hicds, "_", exprds, "_", "cumulative_abs_fc_genes.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(
    x = gene_fc_ranks,
    y = gene_fc_cumsum,
  ...
  outFile <- file.path(outFold, paste0(hicds, "_", exprds, "_", "cumulative_abs_fc_bothGenesTADs.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(
    x = tad_fc_ranks_norm,
  ...
  outFile <- file.path(outFold, paste0(hicds, "_", exprds, "_", "cumulative_abs_corr_tads.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(
    x = tad_corr_ranks,
    y = tad_corr_cumsum,
  ...
  outFile <- file.path(outFold, paste0(hicds, "_", exprds, "_", "cumulative_abs_corr_genes.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(
    x = gene_corr_ranks,
    y = gene_corr_cumsum,
  ...
  outFile <- file.path(outFold, paste0(hicds, "_", exprds, "_", "cumulative_abs_corr_bothGenesTADs.", plotType))
  do.call(plotType, list(outFile, height=myHeight, width=myWidth))
  plot(
