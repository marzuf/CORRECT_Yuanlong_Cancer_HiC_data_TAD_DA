...
script_name <- "distRanking_with_permut_sub.R"
...
outFold <- "DISTRANKING_WITH_PERMUT_SUB"
dir.create(outFold)
...
### SUBSET ONLY THE FIRST nSub PERMUT(TOO SLOW OTHERWISE)
...
all_ds_canberraDist <- foreach(fc_permut_file = all_permutFC_files) %dopar% {
  ...
  meanFC_permDT <- meanFC_permDT[,1:nSub]
  ...
  meanCorr_permDT <- meanCorr_permDT[,1:nSub]
  ...
  cat("... compute meanCorr canberraDist\n")
  ...
  cat("... compute meanFC canberraDist\n")
  ...
  cat("... compute avgRank canberraDist\n")
